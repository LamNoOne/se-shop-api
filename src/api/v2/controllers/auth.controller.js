...
const signIn = asyncHandling(async (req, res) => {
  ...
  new SuccessResponse({
    ...
    metadata: { user: result.user, accessToken: result.accessToken, refreshToken: result.refreshToken}
  }).send(res)
})
...
const refreshToken = asyncHandling(async (req, res) => {
  ...
  new SuccessResponse({
    ...
    metadata: { accessToken: tokenPair.accessToken, refreshToken: tokenPair.refreshToken }
  }).send(res)
})

const signOut = asyncHandling(async (req, res) => {
  const accessToken = getToken(req)
  if (!accessToken) throw new ApiError(StatusCodes.BAD_REQUEST, 'Not signed in yet')
