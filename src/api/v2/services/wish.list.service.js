...
const wishListRepo = require("~/api/v2/repositories/wish.list.repo")
const wishListDetailService = require("~/api/v2/services/wish.list.detail.service")
...
const addProductToWishList = async ({ userId = null, productId = null }) => {
    try {
        ...
        throw new ApiError(
            ...
            "Add product to wish list failed"
        )
    }
}
...
const getMyWishList = async (
    ...
    try {
        const foundWishList = await wishListRepo.getWishList({ userId })
        return await wishListDetailService.getWishListDetails(
            { wishListId: foundWishList.id },
            { filter, selector, pagination, sorter }
        )
    } catch (error) {
...
const deleteProductFromWishList = async ({ userId, productId }) => {
    const foundWishList = await wishListRepo.getWishList({ userId })
    return await wishListDetailService.deleteProductFromWishList({
        wishListId: foundWishList.id,
        productId,
    })
}
