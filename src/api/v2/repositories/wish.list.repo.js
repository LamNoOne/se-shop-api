...
const { WishList, WishListDetail, Product } = require("~/api/v2/models")
...
const getWishList = async ({ userId = null }) => {
    try {
        ...
        if (!foundWishList)
            throw new ApiError(StatusCodes.NOT_FOUND, "Wish list not found")
        return foundWishList
    } catch (error) {
        if (!error.statusCode)
            throw new ApiError(StatusCodes.BAD_REQUEST, "Get wish list failed")
        throw error
    }
}
