...
const tokenRepo = require("~/api/v2/services/token.service")
...
const { verifyToken, getToken } = require("~/api/v2/utils/auth.util")
...
const authenticate = asyncHandling(async (req, res, next) => {
    ...
    try {
        ...
        const foundPermission =
            await permissionRepo.getPermissionByApiMethodVersion({
                api: reqApi,
                method: reqMethod,
                version: reqVersion,
            })
        ...
        const foundToken = await tokenRepo.getTokenByAccessToken({
            accessToken,
        })
        if (!foundToken)
            throw new ApiError(
...
const authorize = asyncHandling(async (req, res, next) => {
    ...
    const accessControlList =
        await rolePermissionRepo.getAllRolePermissionsByRoleId(roleId, {})
